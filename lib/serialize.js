...
async function serialize(msg, conn, store) {
  msg.decodeJid = (jid) => {
    if (/:\d+@/gi.test(jid)) {
  ...
  conn.forward = async (jid, message, forceForward = false, options = {}) => {
    let vtype;
  ...
  if (msg.message) {
    ...
    try {
      ...
      msg.quoted.senderName = await conn.getName(msg.quoted.sender);
    ...
    conn.sendFile = async (jid, path, options = {}) => {
      ...
      /**
       * @type {import('@sampandey001/baileys').proto.WebMessageInfo}
       */
      let m;
      try {
